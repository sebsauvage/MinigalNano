        ...
        if ($file != "." && $file != ".." && $file != "folder.jpg")
        {
            ...
            if (preg_match("/.jpg$|.gif$|.png$/i", $file))
            {
                ...
                if ($lazyload) {
                    $files[] = array (
                        ...
                        "rss" => "<item><title>".$file."</title><link>".$gallery_link."/?dir=".$requestedDir."</link><description>".htmlentities($img_captions[$file])."<img  src='" . GALLERY_ROOT . "createthumb.php?filename=" . $thumbdir . "/" . $file . "&amp;size=$thumb_size' alt='$label_loading' /></description></item>/n");
                } else {
                    $files[] = array (
                        "name" => $file,
                        "date" => filemtime($currentdir . "/" . $file),
            ...
            if ($extension != "")
            {
                $files[] = array (
                    ...
                    "rss" => "<item><title>".$file."</title><link>".$gallery_link."/?dir=".$requestedDir."</link></item>/n");
            }
        }   		
    }
    closedir($handle);
...
{
    for ($i = 0; $i < min(sizeof($files), $nb_items_rss); $i++)
    {
